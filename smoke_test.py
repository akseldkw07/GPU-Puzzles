...
k_map[(1,), (SIZE,)](out, a)
total += 1
passed += check("01 Map", out, map_spec(a))
...
total += 1
passed += check("02 Zip", out, zip_spec(a, b))
...
total += 1
passed += check("03 Guard", out, map_spec(a))
...
total += 1
passed += check("04 Map 2D", out, map_spec(a))
...
total += 1
passed += check("05 Broadcast", out, zip_spec(a, b))
...
total += 1
passed += check("06 Blocks", out, map_spec(a))
...
total += 1
passed += check("07 Blocks 2D", out, map_spec(a))
...
total += 1
passed += check("08 Shared", out, map_spec(a))
...
total += 1
passed += check("09 Pooling", out, pool_spec(a))
...
total += 1
passed += check("10 Dot", out, np.array([dot_spec(a, b)]))
...
total += 1
passed += check("11 Conv (Simple)", out, conv_spec(a, b))
...
total += 1
passed += check("11 Conv (Full)", out, conv_spec(a, b))
...
total += 1
passed += check("12 Sum (Simple)", out, sum_block_spec(a, TPB_SUM))
...
total += 1
passed += check("12 Sum (Full)", out, sum_block_spec(a, TPB_SUM))

# 13
BATCH, SIZE = 4, 6
a = np.arange(BATCH * SIZE).reshape(BATCH, SIZE)
out = np.zeros((BATCH, 1))
k_axis_sum[(1, BATCH), (TPB_AXIS, 1)](out, a, SIZE)
total += 1
passed += check("13 Axis Sum", out, axis_sum_spec(a, TPB_AXIS))
...
total += 1
passed += check("14 Matmul (Simple)", out, matmul_spec(inp1, inp2))
...
total += 1
passed += check("14 Matmul (Full)", out, matmul_spec(inp1, inp2))
